...
exports.import = function(config) {
  ...
  // uncomment this header when importing on Nuxeo < 7.4
  // client.header('X-NXDocumentJsonLegacy', true);
  ...
  var uploadBlob = function(file, doc, metadata, callback) {
    ...
    // upload the file
    uploader.uploadFile(fs.createReadStream(file), function(err, data) {
      if (err) {
        if (config.verbose) {
